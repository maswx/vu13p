...
uint32_t mem_read(void *mem, uint32_t offset);
uint32_t mem_write(void *mem, uint32_t offset, uint32_t val);
uint32_t pcie_iic_init(void *mem);
uint32_t pcie_iicreg_write(void *mem, uint32_t devaddr, uint32_t regaddr, uint32_t regval);
uint32_t pcie_iicreg_read(void *mem, uint32_t devaddr, uint32_t regaddr);
...
//写一个函数，对内存基本地址与偏移地址的和值读取，读取结果为32位无符号整数
uint32_t mem_read(void *mem, uint32_t offset)
{
    return *(volatile uint32_t *)(mem+ offset);
}
uint32_t mem_write(void *mem, uint32_t offset, uint32_t val)
{
    *(volatile uint32_t *)(mem+ offset) = val;
    return 0;
}
